...
function replyTo(text, addToCart) {
  ...
  if (lower.includes('blast') || lower.includes('paddy')) {
    return <>🌾 <strong>Paddy Blast Defense:</strong> We recommend <strong>Sathya Bio BlastShield 75 WP</strong> (₹680) or <strong>Pseudomonas 1% WP</strong>.<br />{cartButton('sb-01', 'Add BlastShield to Cart')}</>
  }
  if (lower.includes('whitefly') || lower.includes('cotton')) {
    return <>🐛 <strong>Cotton Whitefly Defense:</strong> Use <strong>Sathya Bio FlyKill Ultra</strong> (₹840) or <strong>NeemGuard 10000 PPM</strong> (₹580). Spray early morning.<br />{cartButton('sb-02', 'Add FlyKill Ultra to Cart')}</>
  }
  ...
  return <>🌿 <strong>Sathya Bio Crop Assistant:</strong> We offer 35+ bio-certified pesticides and crop nutrients for Paddy, Cotton, Tomato, Wheat, Sugarcane, and Grapes. Filter products by crop or disease above!</>
}
